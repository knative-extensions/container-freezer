...
if [[ -n "${TAG:-}" ]]; then
  LABEL_YAML_CMD=(sed -e "s|container-freezer.knative.dev/release: devel|container-freezer.knative.dev/release: \"${TAG}\"|" -e "s|app.kubernetes.io/version: devel|app.kubernetes.io/version: \"${TAG:1}\"|")
else
  LABEL_YAML_CMD=(cat)
fi
