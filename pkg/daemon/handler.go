...
func (h *Handler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	switch m.Action {
	case "pause":
		h.Logger.Infof("pause request received, freezing pod: %s", resp.Status.User.Extra["authentication.kubernetes.io/pod-name"][0])
		h.Freezer.Freeze(r.Context(), resp.Status.User.Extra["authentication.kubernetes.io/pod-uid"][0], "user-container")
	case "resume":
		h.Logger.Infof("resume request received, thawing pod: %s", resp.Status.User.Extra["authentication.kubernetes.io/pod-name"][0])
